...
const Reunion: React.FC = () => {
  ...
  useEffect(() => {
    axios.get(`https://meetflowback-production.up.railway.app/diagrama/obtenerDiagramaIdReunion/${id}`)
      .then(async (response) => {
        ...
        if (tipo === 'unirse' || tipo === 'nueva' || (location.state && location.state.usuarioId === response.data.usuarioId)) {
          // await axios.post(`https://meetflowback-production.up.railway.app/colaborador/agregar`, { //Registramos al usuario como colaborador
  ...
  const downloadSvg = () => {
    if (diagramRef.current) {
      ...
      if (diagram) {
        ...
        axios.post('https://meetflowback-production.up.railway.app/reuniones/savesvg', { svgString: svgText, id })
          .then(_response => {
  ...
  const handleConvertJavaButtonClick = () => {
    if (diagramRef.current) {
      ...
      if (diagram) {
        ...
        axios.post('https://meetflowback-production.up.railway.app/reuniones/java', requestData)
          .then(response => {
  ...
  const handleConvertPythonButtonClick = () => {
    if (diagramRef.current) {
      ...
      if (diagram) {
        ...
        axios.post('https://meetflowback-production.up.railway.app/reuniones/python', requestData)
          .then(response => {
            // Obtener el contenido de texto del servidor
  ...
  const handleConvertJavaScriptButtonClick = () => {
    if (diagramRef.current) {
      ...
      if (diagram) {
        ...
        axios.post('https://meetflowback-production.up.railway.app/reuniones/javascript', requestData)
          .then(response => {
            // Obtener el contenido de texto del servidor
